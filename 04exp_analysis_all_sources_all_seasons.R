#calculate the lamda for each source (S,C,P) regardless of seasons(4seasons), since control X is not real pathogen
 ...
 for (method in unique(UnewRunoff5$method)) {
     ...
     if(length(indxnew2) > 0)
     {
       ...
       if (!is.null(select.mean_week_new1))
       {
         ...
         lamda<- my.glm.exp.lamda(as.numeric(select.mean_week_new1[,2]),as.numeric(select.mean_week_new1[,1]))
         lamda.intercept<- my.glm.exp.lamda.intercept(as.numeric(select.mean_week_new1[,2]),as.numeric(select.mean_week_new1[,1]))
        lamda1<-format(as.numeric(log(2)/-lamda),digits=3)
         print(lamda1)
         #temp.look <- cbind(select.mean_week1[,1],select.mean_week1[,2])
        hislook_new<- rbind(hislook_new,cbind(as.numeric(lamda1), lamda,lamda.intercept,method,Source))
         ...
         plot(jitter(as.numeric(select.mean_week_new1[,1]))~select.mean_week_new1[,2],type="p",xlab="week", ylab="concentration",
           main=paste(Source,"\n",method,"\n","half-life=",as.numeric(lamda1)*7, "days"))
     ...
     if(length(indxnew1) > 0)
     {
       ...
       if (!is.null(select.mean_week_new1))
       {
        ...
        #lamda1<-format(as.numeric(log(2)/-lamda),digits=3)
         #print(lamda1)
         #temp.look <- cbind(select.mean_week1[,1],select.mean_week1[,2])
         #hislook<- rbind(hislook,cbind(as.numeric(lamda1), method))
         #print(paste(dim(temp.look)[[1]],dim(temp.look)[[2]],season,method))
         plot(jitter(as.numeric(select.mean_week_new1[,1]))~select.mean_week_new1[,2],type="p",xlab="week", ylab="concentration",
           main=paste(method,"\n","half-life=",as.numeric(lamda1)*7, "days"))
